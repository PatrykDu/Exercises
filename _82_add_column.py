class Matrix:
    ...
    def add_column(self, column, index=None):
        if not isinstance(column, list):
            raise TypeError('New column must be a list')
        for item in column:
            if type(item) not in (float, int):
                raise TypeError('Elements of the new column must be all ints or floats')
        if not len(column) == self.n_rows:
            raise ValueError('')
